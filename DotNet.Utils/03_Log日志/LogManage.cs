...
namespace DotNet.Utils
{
     class LogManage
    {
        ...
        public LogManage(string logPath, string logFileExtName, bool writeLogTime)
        {
        ...
        private void WriteLog(string logFile, string msg)
        {
            lock (obj)
            {
                try
                {
                    ...
                    { 
                        logFileName = string.Format("{0}{1}_{2}.{3}", this.LogPath, dateString, logFile, this.logFileExtName);
                    }
